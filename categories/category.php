...
$select = $conn->query("SELECT * FROM props WHERE end_datetime > NOW() ORDER BY name DESC");
$select->execute();
$props = $select->fetchAll(PDO::FETCH_OBJ);
  ...
  if(isset($_GET['type'])) {
    ...
    $rent = $conn->query("SELECT * FROM props WHERE type='$type' AND end_datetime > NOW()");
    $rent->execute();
  ...
  if(isset($_GET['price'])) {
    ...
    $price_query = $conn->query("SELECT * FROM props WHERE end_datetime > NOW() ORDER BY price $price");
    $price_query->execute();
  ...
  if(isset($_GET['name'])) {
    ...
    $singleCategory = $conn->query("SELECT * FROM props WHERE home_type = '$name' AND end_datetime > NOW()");
    $singleCategory->execute();
    $allSingleCategory = $singleCategory->fetchAll(PDO::FETCH_OBJ);
    

    //var_dump($allSingleCategory);
  } else {
    echo "<script>window.location.href='".APPURL."/404.php' </script>";
...
?>
    ...
    <div class="site-section site-section-sm bg-light">
      <div class="container">
        ...
        <div class="row mb-5">
        <?php if (empty($allSingleCategory)) : ?>
        <div class="col-md-12">
            <div class="alert alert-info">
                <h4 class="alert-heading">No Listings Found</h4>
                <p>We're sorry, but there are currently no listings matching your search criteria. Please try again with different filters.</p>
            </div>
        </div>
    <?php else : ?>
        <?php foreach($allSingleCategory as $homeType) : ?>
            ...
            <div class="property-entry h-100">
              ...
              <div class="p-4 property-body">
                ...
                <strong class="property-price text-primary mb-3 d-block text-success">RM<?php echo $homeType->price; ?></strong>
